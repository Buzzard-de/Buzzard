...
const seoGooglePrep = require("../lib/seoGooglePrep");
const i18nCatalog = require("../lib/i18nCatalog");
...
module.exports = {
  register(app) {
    ...
    app.get("/api/p1/seo/status", (_req, res) => {
      return res.json({ success: true, report: seoGooglePrep.getSeoReadinessReport() });
    });

    app.get("/api/p1/i18n/gaps", (req, res) => {
      return res.json({
        success: true,
        report: i18nCatalog.detectProductI18nGaps({
    ...
    app.get("/api/admin/p1/seo/status", (req, res) => {
      if (!requireAuth(req, res)) return;
      if (!requirePermission(req, res, "products.read")) return;
      return res.json({ success: true, report: seoGooglePrep.getSeoReadinessReport() });
    });

    app.get("/api/admin/p1/i18n/gaps", (req, res) => {
      if (!requireAuth(req, res)) return;
      if (!requirePermission(req, res, "products.read")) return;
      return res.json({
        success: true,
