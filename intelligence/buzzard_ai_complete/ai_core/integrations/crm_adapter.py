...
from buzzard_ai_complete.ai_core.integrations.connectors.buzzard_crm import CrmConnector


class CrmAdapter(IntegrationAdapter):
    integration_id = "crm"

    def __init__(self, connector: CrmConnector | None = None) -> None:
        self._connector = connector or CrmConnector()
    ...
    def health_check(self) -> dict[str, Any]:
        if not self.is_configured():
            return {
                ...
                "message": "CRM_API_URL and CRM_API_TOKEN required",
            }
    ...
    def get_customer_context(self, *, customer_ref: str) -> dict[str, Any]:
        return self._connector.get_customer_context(customer_ref=customer_ref)
