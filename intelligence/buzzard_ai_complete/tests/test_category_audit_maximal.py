...
def test_full_live_input():
    ...
    assert live["main_category_count"] == 41
    assert len(live["categories"]) == 41
...
def test_tire_move_migration_item():
    ...
    assert row["parent"] == "Automotive"


def test_energie_solar_separate():
    report = CategoryAuditService().audit_report()["categories"]
    row = next(x for x in report if x["name"] == "Energie & Solar")
    assert row["action"] == "SEPARATE"
    assert row["target"] == "Güneş & Rüzgâr Enerjisi"
...
def test_service_health_full_input():
    ...
    assert health["summary"]["live_main_categories"] == 41
...
def test_sync_from_storefront():
    ...
    assert result["main_category_count"] == 41
