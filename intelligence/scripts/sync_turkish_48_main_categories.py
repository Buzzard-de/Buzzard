#!/usr/bin/env python3
"""Sync Turkish 48 main categories across master taxonomy and Category Intelligence 47."""
...
CANONICAL = REPO / "data/taxonomy/buzzard_master_48_main_categories_tr.json"
CONFIG_PATH = (
    REPO
...
def update_taxonomy_l1(payload: dict) -> None:
    ...
    print(f"OK: taxonomy L1 updated ({updated} field changes)")
...
def update_manifest(categories: list[dict], payload: dict) -> None:
    ...
    manifest["scope"] = "47 non-Kfz categories (Turkish master taxonomy 48)"
    manifest["main_categories_locale"] = payload.get("locale", "tr")
    manifest["main_categories_json"] = "/taxonomy/buzzard_master_48_main_categories_tr.json"
...
def copy_canonical_public() -> None:
    public_path = REPO / "public/taxonomy/buzzard_master_48_main_categories_tr.json"
...
def main() -> None:
    ...
    run_builds()
    print(f"OK: {len(categories)} research categories → {CONFIG_PATH}")
    print(f"OK: canonical → {CANONICAL}")
