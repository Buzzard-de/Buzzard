...
export function getGoLiveSafetyCounters() {
  return { ...counters };
}

export function resetGoLiveSafetyCountersForTests(): void {
  for (const key of Object.keys(counters) as Array<keyof typeof counters>) {
    counters[key] = 0;
  }
}

export function assertGoLiveNetworkSafety(): void {
  if (isSupplierOrderNetworkEnabled()) {
    counters.realSupplierHttpCalls++;
    throw new Error("GO_LIVE_FAIL:SUPPLIER_ORDER_NETWORK_MUST_BE_DISABLED");
  }
}

export function assertGoLiveSafetyInvariants(): { ok: boolean; violations: string[] } {
