...
const mod = require("../lib/analyticsV39");
...
module.exports = {
  register(app) {
    if (!mod.isEnabled()) {
      console.log("Analytics & BI disabled (BUZZARD_ANALYTICS_V39=0 or BUZZARD_DB_ENABLED=0)");
      return;
    }

    app.post("/api/analytics-v39/records", (req, res) => {
      const result = mod.createRecord(req.body || {});
      if (result.error) return res.status(result.status || 400).json({ error: result.error });
      return res.status(201).json(result.record);
    });

    app.get("/api/analytics-v39/records", (req, res) => {
      return res.json(mod.listRecords());
    });

    app.get("/api/analytics-v39/records/:code", (req, res) => {
      const record = mod.getRecordByCode(req.params.code);
      if (!record) return res.status(404).json({ error: "not found" });
      return res.json(record);
    });

    app.patch("/api/analytics-v39/records/:id", (req, res) => {
    ...
    app.post("/api/analytics-v39/jobs", (req, res) => {
      return res.status(202).json(mod.createJob(req.body || {}));
    });

    app.get("/api/analytics-v39/jobs", (req, res) => {
      return res.json(mod.listJobs());
    });

    app.get("/api/admin/analytics-v39/overview", (req, res) => {
      if (!requireAnyAdmin(req, res)) return;
      return res.json(mod.getAnalyticsV39Overview());
    });
  },
};
