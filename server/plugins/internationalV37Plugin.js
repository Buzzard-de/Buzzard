...
const mod = require("../lib/internationalV37");
...
module.exports = {
  register(app) {
    if (!mod.isEnabled()) {
      console.log("Europe & International disabled (BUZZARD_INTERNATIONAL_V37=0 or BUZZARD_DB_ENABLED=0)");
      return;
    }

    app.post("/api/international-v37/records", (req, res) => {
      const result = mod.createRecord(req.body || {});
      if (result.error) return res.status(result.status || 400).json({ error: result.error });
      return res.status(201).json(result.record);
    });

    app.get("/api/international-v37/records", (req, res) => {
      return res.json(mod.listRecords());
    });

    app.get("/api/international-v37/records/:code", (req, res) => {
      const record = mod.getRecordByCode(req.params.code);
      if (!record) return res.status(404).json({ error: "not found" });
      return res.json(record);
    });

    app.patch("/api/international-v37/records/:id", (req, res) => {
    ...
    app.post("/api/international-v37/jobs", (req, res) => {
      return res.status(202).json(mod.createJob(req.body || {}));
    });

    app.get("/api/international-v37/jobs", (req, res) => {
      return res.json(mod.listJobs());
    });

    app.get("/api/admin/international-v37/overview", (req, res) => {
      if (!requireAnyAdmin(req, res)) return;
      return res.json(mod.getInternationalV37Overview());
    });
  },
};
