...
const mod = require("../lib/logisticsV34");
...
module.exports = {
  register(app) {
    if (!mod.isEnabled()) {
      console.log("Logistics & Shipping disabled (BUZZARD_LOGISTICS_V34=0 or BUZZARD_DB_ENABLED=0)");
      return;
    }

    app.post("/api/logistics-v34/records", (req, res) => {
      const result = mod.createRecord(req.body || {});
      if (result.error) return res.status(result.status || 400).json({ error: result.error });
      return res.status(201).json(result.record);
    });

    app.get("/api/logistics-v34/records", (req, res) => {
      return res.json(mod.listRecords());
    });

    app.get("/api/logistics-v34/records/:code", (req, res) => {
      const record = mod.getRecordByCode(req.params.code);
      if (!record) return res.status(404).json({ error: "not found" });
      return res.json(record);
    });

    app.patch("/api/logistics-v34/records/:id", (req, res) => {
    ...
    app.post("/api/logistics-v34/jobs", (req, res) => {
      return res.status(202).json(mod.createJob(req.body || {}));
    });

    app.get("/api/logistics-v34/jobs", (req, res) => {
      return res.json(mod.listJobs());
    });

    app.get("/api/admin/logistics-v34/overview", (req, res) => {
      if (!requireAnyAdmin(req, res)) return;
      return res.json(mod.getLogisticsV34Overview());
    });
  },
};
