...
const mod = require("../lib/orderManagementV32");
...
module.exports = {
  register(app) {
    if (!mod.isEnabled()) {
      console.log("Order Management System disabled (BUZZARD_ORDER_MANAGEMENT_V32=0 or BUZZARD_DB_ENABLED=0)");
      return;
    }

    app.post("/api/order-management-v32/records", (req, res) => {
      const result = mod.createRecord(req.body || {});
      if (result.error) return res.status(result.status || 400).json({ error: result.error });
      return res.status(201).json(result.record);
    });

    app.get("/api/order-management-v32/records", (req, res) => {
      return res.json(mod.listRecords());
    });

    app.get("/api/order-management-v32/records/:code", (req, res) => {
      const record = mod.getRecordByCode(req.params.code);
      if (!record) return res.status(404).json({ error: "not found" });
      return res.json(record);
    });

    app.patch("/api/order-management-v32/records/:id", (req, res) => {
    ...
    app.post("/api/order-management-v32/jobs", (req, res) => {
      return res.status(202).json(mod.createJob(req.body || {}));
    });

    app.get("/api/order-management-v32/jobs", (req, res) => {
      return res.json(mod.listJobs());
    });

    app.get("/api/admin/order-management-v32/overview", (req, res) => {
      if (!requireAnyAdmin(req, res)) return;
      return res.json(mod.getOrderManagementV32Overview());
    });
  },
};
